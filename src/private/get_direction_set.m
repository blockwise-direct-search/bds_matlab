...
%
%   If the user do not input OPTIONS or the OPTIONS does not contain direction_set.
...
%   2. Find those directions which are almost parallel. Then preserve the first one and remove the others.
...
if ~isfield(options, "direction_set")
    ...
    % We remove the directions that are parallel, not appearing for the first time.
    ...
    % If rank(direction_set) < n, we add new columns in the direction set to make it linearly span the full space.
    % We use QR factorization. In particular,make the diagonal elements of matrix R monotonically decreasing
    % by introducing the permutation matrix P. 
    [Q, R, ~] = qr(direction_set);
    [~, m] = size(R);

    % We preserve the columns in D where the diagonal elements of R are not too small correspondingly. 
    % Then We need to consider two types of matrix R.
    % 1. R is a vector. Then we only select the first element of R to see whether it is larger than the tolerance. 
    % 2. R is a matrix but not vector. Then we select the transpose of the diag(R) since the vecnorm will return
    %    a column vector.
    if min(m, n) == 1
        rank_direction_set_clean = sum(abs(R(1, 1)) > 10*eps*max(m,n)*R(1, 1));
    else
    ...
    direction_set = [direction_set, Q(:, rank_direction_set_clean+1:end)];
    ...
    D(:,1:2:2*direction_set_row_num-1) = direction_set;
    D(:,2:2:2*direction_set_col_num) = -direction_set;
